#!/usr/bin/env python
"""
Plot each species minimum psi_leaf as a map for each of the three experiments
(CTL, rPPT and eCO2) 
...
def main(df, experiment, species_list, nice_species_list, ofname, plot_dir):
    ...
    #cmap = plt.cm.get_cmap('YlOrRd_r') # continuous colour map
    ...
    for i, ax in enumerate(axgr):
        ...
        for k in range(len(species_mtch)):
            data[int(species_mtch["row"][k]),\
                 int(species_mtch["col"][k])] = species_mtch["psi_leaf_min"][k]
    ...
    cbar.ax.set_title("$\Psi$$_{\mathrm{min}}$ (MPa)", fontsize=16, pad=10)
